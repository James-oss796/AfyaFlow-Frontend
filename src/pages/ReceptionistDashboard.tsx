...
const ReceptionistDashboard: React.FC = () => {
  ...
  const activeToken = patients.find(p => p.status === 'in-progress') || queuePatients[0];
  const queueWaiting = patients.filter(p => p.status === 'queued');

  const avgWait = queueWaiting.length > 0 ? Math.round(queueWaiting.length * 3.5) : 0;
  const dailyTokens = patients.length;
