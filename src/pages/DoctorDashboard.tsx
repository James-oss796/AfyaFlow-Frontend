...
const DoctorDashboard: React.FC = () => {
  const { patients, updatePatientStatus, addPrescription, addReferral, getDepartmentStats } = useData();
  ...
  // Get queue: in-progress first, then queued
  ...
  const activePatient: Patient | undefined = patients.find(p => p.status === 'in-progress') || queue[0];
  ...
  const servedToday = patients.filter(p => p.status === 'served').length;
      ...
      <section className="col-span-12 lg:col-span-4 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-on-surface-variant">{queue.length} waiting · {servedToday} served today</p>
          </div>
          <SignatureButton
            variant="primary"
            icon="play_arrow"
        ...
        <div className="space-y-3">
          {queue.length === 0 ? (
            <div className="p-8 text-center bg-surface-container-lowest rounded-2xl">
              ...
              <p className="text-xs text-on-surface-variant mt-1">All patients have been seen</p>
            </div>
          ) : queue.map((patient, i) => (
            <div
              key={patient.id}
        ...
        <div className="bg-surface-container-lowest p-6 rounded-2xl border border-outline-variant/10 shadow-sm">
          <h3 className="text-xs font-bold uppercase tracking-wider text-on-surface-variant mb-4 flex items-center gap-2">
            ...
            Department Load ({Object.keys(deptStats).length})
          </h3>
          <div className="space-y-3">
            {Object.entries(deptStats).sort((a, b) => b[1] - a[1]).map(([dept, count]) => (
              <div key={dept} className="flex justify-between items-center text-sm">
                ...
                <span className="font-bold text-primary bg-primary/10 px-2 py-0.5 rounded">{count} waiting</span>
              </div>
            ))}
            {Object.keys(deptStats).length === 0 && (
              <p className="text-xs text-on-surface-variant italic">No patients waiting.</p>
            )}
          </div>
        </div>
      </section>
