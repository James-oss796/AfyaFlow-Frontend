...
const LoginPage: React.FC = () => {
  ...
  const { login } = useAuth(); // removed unused 'user'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (success) {
        // After successful login, determine the correct dashboard based on role
        setTimeout(() => {
          const currentUser = JSON.parse(localStorage.getItem('afyaflow_session') || 'null');
          if (currentUser) {
