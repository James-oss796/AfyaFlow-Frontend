...
const allNavItems = [
  { icon: 'how_to_reg', label: 'Reception', path: '/', roles: ['Admin', 'Receptionist'] },
  { icon: 'admin_panel_settings', label: 'Admin', path: '/admin', roles: ['Admin'] },
  { icon: 'stethoscope', label: 'Doctor', path: '/doctor', roles: ['Admin', 'Doctor'] },
  { icon: 'person', label: 'Patients', path: '/patients', roles: ['Admin'] },
...
const SideNavBar: React.FC = () => {
  ...
  return (
    <aside className="h-screen w-64 fixed left-0 top-0 z-40 flex flex-col py-6 border-r border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 font-manrope antialiased tracking-tight text-sm font-medium">
      <div className="text-xl font-bold text-teal-900 dark:text-teal-100 px-4 mb-8">
        Afyaflow
      </div>
      ...
      <div className="px-4 mt-auto">
        <div className="bg-surface-container-high rounded-xl p-4 flex flex-col gap-2">
          <div className="flex items-center justify-between">
            ...
            <div className="h-2 w-2 rounded-full bg-secondary shadow-[0_0_8px_rgba(44,105,78,0.6)]"></div>
          </div>
